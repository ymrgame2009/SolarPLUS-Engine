...
class UpdateState extends MusicBeatState
{
	...
	override function create()
	{
		...
		progressText.setFormat(Paths.font('funkin.ttf'), 16, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		add(progressText);

		download_info = new FlxText(progressBar.x + progBar_bg.width, progressBar.y + progBar_bg.height, 0, "0B / 0B", 16);
		download_info.setFormat(Paths.font('funkin.ttf'), 16, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		add(download_info);
	...
	inline function getUpdateLink()
	{
		...
		online_url = "https://github.com/VideoBotYT/Universe-Engine/releases/download/" + TitleState.updateVersion + '/FNF-Universe-Engine-${getPlatform()}.$fileEnd';
