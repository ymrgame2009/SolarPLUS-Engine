...
class NotesSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			setShaderColor(!onPixel ? deafaultArrowcolor[curSelectedNote][curSelectedMode] : ClientPrefs.defaultPixelRGB[curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
	...
	public function spawnNotes()
	{
		...
		for (i in 0...dataArray.length)
		{
			...
			if (onPixel)
			{
				...
				newNote.animation.add('static', [i]);
				newNote.animation.add('pressed', [i + 4], 12, false);
				newNote.animation.add('confirm', [i + 8], 24, false);
