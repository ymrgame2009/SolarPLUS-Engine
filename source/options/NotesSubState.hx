...
class NotesSubState extends MusicBeatSubstate
{
    ...
    public function new() {
        ...
        tip.setFormat(Paths.font("funkin.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        tip.borderSize = 2;
        add(tip);
        ...
        FlxG.mouse.visible = !controllerMode;
        controllerPointer.visible = controllerMode;
        _lastControllerMode = controllerMode;
    }

    function updateTip()
    {
        tipTxt.text = 'Hold ' + (!controllerMode ? 'Shift' : 'Left Shoulder Button') + ' + Press RESET key to fully reset the selected Note.';
    }
    ...
    override function update(elapsed:Float) {
        ...
        if(FlxG.gamepads.anyJustPressed(ANY)) controllerMode = true;
        else if(FlxG.mouse.justPressed || FlxG.mouse.deltaScreenX != 0 || FlxG.mouse.deltaScreenY != 0) controllerMode = false;
        //
        
        var changedToController:Bool = false;
        if(controllerMode != _lastControllerMode)
        {
            FlxG.mouse.visible = !controllerMode;
            controllerPointer.visible = controllerMode;
        ...
        if(controllerMode && (changedToController || FlxG.gamepads.anyInput()))
        {
        ...
        var controllerPressed:Bool = (controllerMode && controls.ACCEPT);
        //
        ...
        {
            if (FlxG.mouse.justReleased || (controllerMode && FlxG.gamepads.anyJustReleased(FlxGamepadInputID.A)))
            {
                holdingOnObj = null;
                _storedColor = getShaderColor();
                updateColors();
        ...
        else if(controls.RESET && hexTypeNum < 0)
        {
            var defaultColors:Array<FlxColor> = [
                FlxColor.fromString('0xC24B99'), 
                FlxColor.fromString('0x00FFFF'), 
                FlxColor.fromString('0x12FA05'), 
                FlxColor.fromString('0xF9393F')  
            ];
            ...
            if(FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyJustPressed(LEFT_SHOULDER))
            {
                for (i in 0...3)
                {
                    ...
                    var color:FlxColor = defaultColors[curSelectedNote];
                    switch(i)
                    {
                        case 0:
            ...
            setShaderColor(defaultColors[curSelectedNote]);
            FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
            updateColors();
        }
    }
    ...
    {
        if (!controllerMode) return FlxG.mouse.overlaps(obj);
        return FlxG.overlap(controllerPointer, obj);
    }

    function pointerX():Float
    {
        if (!controllerMode) return FlxG.mouse.x;
        return controllerPointer.x;
    }
    function pointerY():Float
    {
        if (!controllerMode) return FlxG.mouse.y;
        return controllerPointer.y;
    }
    function pointerFlxPoint():FlxPoint
    {
        if (!controllerMode) return FlxG.mouse.getScreenPosition();
