...
class PlayState extends MusicBeatState
{
        ...
        override public function create()
        {
                ...
                debugModchartKey = ClientPrefs.copyKey(ClientPrefs.keyBinds.get('debug_3'));
        ...
        public function updateScore(miss:Bool = false)
        {
                scoreTxt.text = 'Score: '
                        + songScore
                        + ' - Screw-Ups: ' // OG: ' - Misses: ' its me YMR!
                        + songMisses
                        + ' - Rating: '
                        + ratingName
                ...
                for (section in noteData)
                {
                        for (songNotes in section.sectionNotes)
                        {
                                ...
                                for (j in 0...Std.int(jackingtime)) {
                                                var jackStrumTime:Float = swagNote.strumTime + (15000 / SONG.bpm) * (j + 1);
        
                                                
                                                var jackNote:Note = new Note(jackStrumTime, swagNote.noteData, null, false, false);
                                                jackNote.mustPress = swagNote.mustPress;
                                                jackNote.noteType = swagNote.noteType;
                                                jackNote.animSuffix = (songNotes[3] == 'Alt Animation' || section.altAnim ? '-alt' : '');
                                                jackNote.gfNote = swagNote.gfNote;
                                                jackNote.isSustainNote = false;
                                                jackNote.hitHealth = swagNote.hitHealth;
                                                jackNote.missHealth = swagNote.missHealth;
                                                //jackNote.wasHit = false;
                                                jackNote.multSpeed = 1;
                                                jackNote.multAlpha = 1;
                                                jackNote.hitCausesMiss = swagNote.hitCausesMiss;
                                                jackNote.ignoreNote = swagNote.ignoreNote;
        ...
        override public function update(elapsed:Float)
        {
                ...
                if (generatedMusic && !inCutscene)
                {
                    ...
                    var oppStrum:StrumNote = opponentStrums.members[daNote.noteData];
                    if (ClientPrefs.noteSplashes && !daNote.noteSplashData.disabled && oppStrum != null)
                    {
                        if (!daNote.isSustainNote && daNote.sustainLength > 0)
                        {
                            spawnHoldSplash(oppStrum, daNote, 'hold');
                        }
                    }
        ...
        function noteMissPress(direction:Int = 1):Void // You pressed a key when there was no notes to press for this key
        {
                ...
                if (!boyfriend.stunned)
                {
                        ...
                        // bro Why this shit here like THAT :sob:?!! "Mr YMR"
        ...
        function goodNoteHit(note:Note):Void
        {
                ...
                var targetStrum:StrumNote = playerStrums.members[note.noteData];
                if (ClientPrefs.noteSplashes && note.sustainLength > 0 && !note.noteSplashData.disabled && targetStrum != null)
                {
                    spawnHoldSplash(targetStrum, note, 'hold');
                }
                        ...
                        vocals.volume = 1;
                        // PLS WORK :sob: "Mr YMR"
                var endStrum:StrumNote = playerStrums.members[note.noteData];
                if (note.isSustainNote && note.animation.curAnim != null && note.animation.curAnim.name.endsWith('end') && endStrum != null)
                {
                    if (ClientPrefs.noteSplashes && !note.noteSplashData.disabled)
                    {
                        spawnHoldSplash(endStrum, note, 'end');
                    }
                }
        ...
        public function spawnHoldSplash(strum:StrumNote, daNote:Note, animMode:String = 'hold'):Void
        {
        ...
        public function RecalculateRating(badHit:Bool = false)
        {
                ...
                if (ret != FunkinLua.Function_Stop)
                {
                        ...
                        // Rating FC 'OR KFC HAHAHAHAHAHA' -> "Mr YMR"
                        ratingFC = "";
                        if (sicks > 0)
                                ratingFC = "SFC";
                        if (goods > 0)
