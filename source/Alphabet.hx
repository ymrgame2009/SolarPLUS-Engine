...
class Alphabet extends FlxSpriteGroup
{
    ...
    public var distanceOffset:FlxPoint = new FlxPoint(0, 0);
    ...
    override function update(elapsed:Float)
    {
        if (isMenuItem)
        {
            ...
            if (changeX)
                x = FlxMath.lerp(x, (targetX * distancePerItem.x) + startPosition.x + distanceOffset.x, lerpVal);
            if (changeY)
                y = FlxMath.lerp(y, (targetY * 1.3 * distancePerItem.y) + startPosition.y + distanceOffset.y, lerpVal);
        }
